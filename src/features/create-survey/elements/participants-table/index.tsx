...
import IndividualActions from './individual-actions';
import {
  ISpan,
...
const CampaignInfluencersTable = ({ id }: { id: number }) => {
  ...
  const renderItem = ({ headItem, row }: TTableRenderItemObject) => {
    ...
    if (
      headItem.reference === 'questionnaireLink' &&
      [
        ProductOrderInfluencerStatus.ToBeApproved,
        ProductOrderInfluencerStatus.Approved,
        ProductOrderInfluencerStatus.NotApproved,
      ].includes(productInfluencer.status) &&
      survey
    ) {
      const linkToAnswers =
        role === 'SUPERADMIN'
          ? `/services/surveys/manage/${survey.id}/${productInfluencer.influencerId}`
          : `/surveys/manage/${survey.id}/${productInfluencer.influencerId}`;
      return (
        <Link href={linkToAnswers}>
          <LockIcon />
        </Link>
      );
    }
    ...
    if (headItem.reference === 'actions' && survey) {
      return (
        <IndividualActions
          surveyId={survey.id}
...
export default CampaignInfluencersTable;
